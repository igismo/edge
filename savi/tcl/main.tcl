...
proc main(build) {} {
    ...
    label $cmd.l -text \
	"no.  semi-major axis  eccentricity  inclination  longitude asc. node  arg. periapsis  time to periapsis  satellite name       type" \
	-font $FONT(label)
...
proc main(constellations_menu) {submenu} {
	...
	{"Empty space..." "empty(build)" "M"} \
        {"GS-Leo(Orb 2 Sat 1)" "main(constellation) GS-Orbit2-Sat1.tcl"} \
	{"GS-Leo(Orb 1 Sat 10)" "main(constellation) GS-Orbit1-Sat10.tcl"} \
	{"GS-Leo(Orb 2 Sat 10)" "main(constellation) GS-Orbit2-Sat10.tcl"} \
	{"GS-Leo(Orb 3 Sat 10)" "main(constellation) GS-Orbit3-Sat10.tcl"} \
...
### GORAN 1 line
    set type [satellites GET_TYPE $i]
...
### GORAN added $type
    set line [list $id [string range $noe 0 $length] $name $type]
