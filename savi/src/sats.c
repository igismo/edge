...
sats_set_cmd(int argc, char *argv[])
{
  ...
  if (s) {
    ...
    if (a < 0) {
      a = -a;
      if (debug) fprintf(stderr,"turned negative semi-major axis into positive a=%f", a);
    }
    ...
    if (e < 0) {
      e = -e;
      if (debug) fprintf(stderr,"turned negative eccentricity into positive e=%f", e);
    }
    if (e > 0.999) e = 0.999;
...
sats_tag_cmd(int argc, char *argv[])
{
  ...
  if (s && !s->tag) {
   ...
   if (debug) fprintf(stderr,"sats: after tagging, tagged %i satellites.",constellation.n_tag_sats);
    if (geomview_module) {
      gv_begin();
      gv_sat_create(s, &constellation);
      gv_end();
...
sats_untag_cmd(int argc, char *argv[])
{
  ...
  if (s && s->tag) {
    s->tag = 0;
    (constellation.n_tag_sats)--;
    if (debug) fprintf(stderr,"sats: after untagging, tagged %i satellites.",constellation.n_tag_sats);
    if (geomview_module) {
      gv_begin();
      gv_sat_create(s, &constellation);
      gv_end();
...
sats_load_marker_cmd(int argc, char *argv[])
{
  ...
  if (current_marker >= EQUATORIAL_MARKERS) {
    current_marker = 0;
    if (debug) {
    	fprintf(stderr,"marker creation wrapped round array - redrawing? %f", Omega * DEG_TO_RAD);
    }
  }
  
  marker[current_marker] = satellite_create(0, &oe, constellation.pcb);
  if (name && !satellite_name(marker[current_marker], name)) {
	  fprintf(stderr,"problem naming marker name=%s", name);
  }
...
//### GORAN
  ...
  if (name && !satellite_name(s, name)) fprintf(stderr,"sats: problem naming satellite name=%s", name);
//### GORAN

if (type && !satellite_type(s, type)) fprintf(stderr,"sats: problem with satellite type type=%s", type);
...
sats_get_name_cmd(int argc, char *argv[])
{
  ...
  if (s) {
    ...
    if (n > SATELLITE_NAME_LENGTH) {
    	fprintf(stderr,"sats: string has overflowed in struct %d name=%s", n, (char *) &s->name);
      return EMPTY_str;
    }
...
///####### GORAN
char *
sats_get_type_cmd(int argc, char *argv[])
{
  Satellite s;
  unsigned int n;
  int i;
  ...
  if (s) {
    n = strlen((char *)&s->type);
    if (n > SATELLITE_NAME_LENGTH) {
    	fprintf(stderr,"sats: the string has overflowed in struct n=%d anme=%s", n, (char *) &s->name);
      return EMPTY_str;
    }
    strcpy(result, (char *) &s->type);

    /* terminate string - elements are 0 to length-1 */
    result[SATELLITE_NAME_LENGTH - 1] = 0;
  }
fprintf(stderr,"sats: call sats_get_type = %s\n", result);
  return result;

}
char *
sats_type_cmd(int argc, char *argv[])
{
  Satellite s;
  int n;
  ...
  if (s) satellite_type(s, type);
fprintf(stderr,"sats: call sats_type_cmd = %s\n", type);
  return EMPTY_str;
}
//##########################################
