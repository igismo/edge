...
// GORAN temp
// fprintf(stderr, "write_satellites_geom:  update=%d\n", satellites_geom_needs_updating);
// use_fast_marker_satellite = FALSE;
// temporary  
...
// fprintf(stderr, "write_satellites_geom: fast=%d\n", use_fast_marker_satellite);
    if (!use_fast_marker_satellite) {
...
//### GORAN
		...
		if (strcmp(s->type, fancy_hname) ||
                       strcmp(s->type, sphere_hname) ||
                       strcmp(s->type, planes_hname) ||
                       strcmp(s->type, box_hname))
		  {
                   fprintf(gv_out, "{INST transform:trans_%d geom:%s}\n", s->id, s->type);
                  }
#endif
//### END GORAN
    ...
    } else {  //  DO USE use_fast_marker_satellite
      ...
      if (num_view_sats > 0) {
       if (GV_BINARY_FORMAT_AVAILABLE) {
		...
		gv_send("}\n");
       } else {  // not GV_BINARY_FORMAT_AVAILABLE
		...
		gv_send("0 1 0 1}\n");
	   } // END of not GV_BINARY_FORMAT_AVAILABLE
      } // END of num_view_sats > 0

      /* write out any tagged satellites */
      if (num_tag_sats > 0) {
fprintf(stderr, "write_satellites_geom: num_tag_sats=%d\n", num_tag_sats);
