...
main(int argc, char *argv[])
{
  ...
  if (geomview_module) {
    if (!gv_stream_init_proc()) {
    	fprintf(stderr,"main: could not open pipe to Geomview; savi is now running standalone.");
      geomview_module = FALSE;
    } else if (!gv_init()) {
    	fprintf(stderr,"main: could not initialise Geomview (main: error in gv_init).");
      geomview_module = FALSE;
    } else {
  ...
  if (!check_env_vars()) {
    error("main: edit and run the savi wrapper script in savi's directory to set locations.");
  }

// GORAN
  fprintf(stderr,"\nmain: Initialize UDP\n");
  if (!udpReceiveInit()) {
     error_and_exit("main: failed to initialize UDP receive");
  }
//  if (!udpSendInit()) {
//     error_and_exit("main: failed to initialize UDP send");
//  }
  // CLOSE SOCKETS ON EXIT !!
  fprintf(stderr,"\nmain: UDP Initialization Done\n");
// End GORAN
  ...
  if (!tcl_init(interp)) {
    error_and_exit("main: could not initialise Tcl (main: error in tcl_init).");
  }

  /* initialize Tk interface */
  if (!tk_init(interp)) {
    error_and_exit("main: could not initialise Tk (main: error in tk_init).");
  }
  ...
  if (!tcl_script(buf)) {
    error_and_exit("main: is SaVi compiled correctly? "
		   "Edit savi startup scripts to set locations.");
  }

  /* loop until exit */
// GORAN added udpReceiveLoop()
  while (sats_update() && tk_update() && udpReceiveLoop());
...
check_env_vars(void)
{
  ...
  if (!getenv("SAVI")) {
	  fprintf(stderr,"main: $SAVI was not set to location of savi directory containing launch script.");
    all_env_found = FALSE;
  }

  if (!getenv("TCL_LIBRARY")) {
	  fprintf(stderr,"main: $TCL_LIBRARY was not set to location of directory containing Tcl.");
    all_env_found = FALSE;
  }

  if (!getenv("TK_LIBRARY")) {
	  fprintf(stderr,"main: $TK_LIBRARY was not set to location of directory containing Tk.");
    all_env_found = FALSE;
  }
...
decode_cmd_line(int argc, char *argv[])
{
  ...
  if (give_help) {
    error("main: supported command-line options when SaVi is run standalone include:\n"
  ...
  if (skipped_filenames) {
    if (skipped_filenames > 1) {
      error("main: couldn't find multiple files passed as parameters.");
    } else {
  ...
  if (script_filename) {
    ...
    if (skipped_filenames || more_filenames) {
      error("main: loading specified file:");
      error(script_filename);
    }
  }

  if (!geomview_module) {
    error("main: For 3D visualization, run from within Geomview.");
    if (!geomview_logo) {
      error("main: logo control is for Geomview.");
      geomview_logo = TRUE;
    }
    if (geomview_texture_with_map) {
      error("main: dynamic texture mapping only under Geomview.");
      geomview_dynamic_texture_flag = FALSE;
      geomview_texture_with_map = FALSE;
    }
    if (geomview_sun_lighting) {
      error("main: sun lighting control is only in Geomview.");
  ...
  if (fake_geomview_module) {
    if (!not_fake_geomview) {
      error("main: Enabling all features that normally require Geomview.");
      sprintf(temp_buffer," -fake-geomview");
      sprintf(switches, temp_buffer);
      switches += strlen(temp_buffer);
    } else {
      error("main: -geomview overrides -fake-geomview. Waiting for Geomview.");
      fake_geomview_module = FALSE;
    }
  }

  if (!can_use_zlib) {
    error("main: compiled without zlib compression.");
  }

  if (levels_error) {
    error("main: Only 1 to 19 levels of color shading are supported.");
  }

  if (levels_format_error) {
    error("main: format is -levels <levels of color shading>. Default is 4.");
  }

  if (NUM_COLORS) {
    if (NUM_COLORS == 1) {
      error("main: one level of color shading selected for all coverage.");
    } else {
      fprintf(stderr, "\nmain: %i levels of color shading selected.",
	      NUM_COLORS);
    }
  ...
  if (orbit_model_unknown) {
    error("main: unfamiliar orbit model. format is -orbit-model <J0|J2>. Selecting J2.");
  } else if (orbit_model == J0) {
    error("main: selected J0 orbital model for classical mechanics.");
    sprintf(temp_buffer, " -orbit-model J0");
    sprintf(switches, temp_buffer);
    switches += strlen(temp_buffer);
  } else if (orbit_model == J2) {
    error("main: defaulting to J2 orbital model.");
  ...
  if (map_format_error) {
    error("main:format is -map-view-middle <longitude>");
  }

  if (map_longitude_error) {
    error("main: longitude given for middle of map view is invalid.");
  }

  if (map_view_error) {
    error("main: format is -map-view-height <number of pixels>");
  }
  if (min_transmit_altitude_error) {
    error("main: format is -min-transmit-altitude <kilometers>");
  }
  if (max_transmit_altitude_error) {
    error("main: format is -max-transmit-altitude <kilometers>");
  }
