...
int satellite_name(Satellite s, char * name)
{
  ...
  if (n < SATELLITE_NAME_LENGTH) {
    ...
    error("Satellite: satellite name too long. Ignored.");
  }

  return TRUE;
}
//######### GORAN
int satellite_type(Satellite s, char * type)
{
  unsigned int n;
  ...
  if (n < SATELLITE_NAME_LENGTH) {
    strcpy((char *)&s->type, type);
  } else {
    error("Satellite: satellite type too long. Ignored.");
  }
